...
see_also: ["architectures/hugging-xberg-mcp/containers/0001-system-container.container.md", "decisions/0005-port-hugging-kreuzberg-to-xberg.decision.md"]
deprecated:
  date: null
  reason: null
  superseded_by: null
...
> **Historical note:** This memory documents the pre-port kreuzberg backend behavior (the original `hugging-kreuzberg-mcp`, see DEC-0005). The xberg backend reports extraction failures differently — check the `errors` / `processing_warnings` keys in the `{results, errors, summary}` envelope. The base64-validation checklist below still applies.
