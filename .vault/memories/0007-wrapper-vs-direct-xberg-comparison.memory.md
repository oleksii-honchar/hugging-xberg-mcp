...
see_also: ["decisions/0005-port-hugging-kreuzberg-to-xberg.decision.md", "concepts/0004-mcp-tool-reference.concept.md"]
deprecated:
  date: null
  reason: null
  superseded_by: null
...
3. **`extract_structured` is server-configured** — schema and model are set via environment variables on the server side, not passed per-call; the tool only takes `data` (DEC-0004).
4. **Fewer tools** — only the two extraction tools; `extract_batch` and cache tools are not available.

## Context

Design rationale from the wrapper docs and the port decision (DEC-0005): the wrapper exists precisely because the native Xberg MCP interface is incompatible with opencode's base64/URI attachment model.
