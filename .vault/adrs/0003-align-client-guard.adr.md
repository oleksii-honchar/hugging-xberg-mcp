...
# ADR-0003: Align Client-Side Size Guard with Server Limit
...
`MAX_BASE64_LENGTH` in `xberg-client.js` must be aligned with the server body limit so the client guard triggers before (or at the same point as) body-parser's 413. The xberg port raised the server limit to 50mb (ADR-0001) to accommodate PDFs.
