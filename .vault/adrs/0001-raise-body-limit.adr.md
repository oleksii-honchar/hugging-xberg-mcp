...
supersedes: []
superseded_by: []
see_also: ["adrs/0002-preserve-status-codes.adr.md", "adrs/0003-align-client-guard.adr.md", "specifications/0001-body-limit-fix.spec.md", "memories/0001-opencode-inline-base64-gotcha.memory.md"]
deprecated:
  date: null
  reason: null
  superseded_by: null
...
# ADR-0001: Raise Express Body Limit to 50mb for Base64 Image/PDF Payloads
...
The MCP server uses `express.json({ limit: process.env.MCP_BODY_LIMIT || '50mb' })`. opencode resolves attachment URIs to inline base64 with no size cap. Base64 encoding adds ~33% overhead: 36.5MB raw → ~48.9MB base64. The port to the xberg backend (ADR-0005) extends extraction to PDFs, which are larger than photos, so the old 20mb limit capped PDFs at ~14MB raw.
